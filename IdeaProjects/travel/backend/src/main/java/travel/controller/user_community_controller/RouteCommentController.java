...
public class RouteCommentController {
    ...
    /**
     * 点赞评论
     * POST /api/route-comments/{commentId}/like
     */
    @PostMapping("/{commentId}/like")
    public Result<Boolean> likeComment(@PathVariable Integer commentId,
                                       @RequestParam Integer userId) {
        try {
            log.info("点赞评论请求: commentId={}, userId={}", commentId, userId);
            boolean result = routeCommentService.likeComment(commentId, userId);
            if (result) {
                return Result.success("点赞评论成功", true);
            }
    ...
    /**
     * 取消点赞评论
     * POST /api/route-comments/{commentId}/unlike
     */
    @PostMapping("/{commentId}/unlike")
    public Result<Boolean> unlikeComment(@PathVariable Integer commentId,
                                         @RequestParam Integer userId) {
        try {
            log.info("取消点赞评论请求: commentId={}, userId={}", commentId, userId);
            boolean result = routeCommentService.unlikeComment(commentId, userId);
            if (result) {
                return Result.success("取消点赞评论成功", true);
            }
            return Result.error("取消点赞评论失败");
        } catch (Exception e) {
            log.error("取消点赞评论失败: commentId={}, error={}", commentId, e.getMessage());
            return Result.error(e.getMessage());
        }
    }
