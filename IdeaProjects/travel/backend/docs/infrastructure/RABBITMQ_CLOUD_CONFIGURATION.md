...
- `newagent` 和 `novel_agent` 中没有 RabbitMQ 配置，两个项目只把 `49.234.187.76` 用作 Milvus 云端地址。
- 本机在 2026-08-10 验证 `49.234.187.76:5672` 和 `49.234.187.76:15672` TCP 可达。
...
| RabbitMQ | 云服务器 | `49.234.187.76:5672` | `RABBITMQ_HOST`、`RABBITMQ_PORT`、`RABBITMQ_VHOST` | 已具备生产端确认和可选发布状态持久化第一阶段，消费端可靠性仍属于 P8.4 未完成项 |
