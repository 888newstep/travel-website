...
若没有 Key，直接展示 `run-logs/amap/20260820-130903/run-summary.json` 的安全失败，并说明真实外呼仍是待验收项。
...
> 状态表具备补偿抢占原语，但当前没有自动定时扫描任务，所以我不会把它描述成完整 Outbox 自动补偿系统。

若没有云凭据，展示 `run-logs/rabbitmq/20260820-133520/run-summary.json` 的预检失败，不声称完成 L4 外部验收。

### 04:45-05:00 总结

结尾建议：

> 这个项目的价值不是功能数量，而是把失败场景做实：重复请求如何收敛、Redis 故障为什么 fail-closed、路线换位如何避免唯一键冲突、消息重试如何避免先 ACK 后丢失，以及外部数据不可用时为什么宁可返回不可用也不造数据。245 项后端测试和四组真实 100 并发结果都可以复现。
