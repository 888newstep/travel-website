...
| Item | Result |
|------|--------|
| Backend test suite | 279 tests, 0 failures, 0 errors, 3 skipped |
...
| Real AMap call | Two-point driving returned 161 live traffic segments; three-point routing returned 55 navigation steps |
| Cloud RabbitMQ | Live publish/consume, deduplication, retry/DLQ, replay, restart, reconnect, and 100-message concurrency all passed |
...
flowchart LR
    ...
    Attraction -.places/weather/facilities.-> AMap
...
| Attraction Service | 8092 | Cities, attractions, reviews, AMap place search, weather, nearby facilities |
| Route Service | 8093 | Routes, schedules, optimization, AMap driving traffic, conditional AI APIs |
...
- AMap place search, weather, nearby facilities, routes, and traffic require a real `AMAP_API_KEY`.
...
| `AMAP_API_KEY` | AMap Web service | Required for places, weather, facilities, and traffic |
| `AMAP_MAX_QPS` | Per-instance AMap limiter | Defaults to `3`, matching the current account quota |
