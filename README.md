...
| 项目 | 结果 |
|------|------|
| 后端全量测试 | 297 tests，0 failure，0 error，0 skipped |
...
| 高德真实外呼 | 两点驾车返回 161 个实时路况分段；三点途经点规划返回 55 个导航步骤 |
| 云 RabbitMQ | 真实发布消费、重复幂等、retry/DLQ、回放、重启、断连恢复和 100 消息并发均通过 |
...
| Attraction Service | 8092 | 城市、景点、点评、高德地点搜索、天气与周边设施 |
| Route Service | 8093 | 路线、日程、优化、高德驾车路况和条件 AI 接口 |
...
- 高德地点搜索、天气、周边设施、路线与路况：需要真实 `AMAP_API_KEY`。
...
| `AMAP_API_KEY` | 高德 Web 服务 | 地点、天气、设施和真实交通必填 |
| `AMAP_MAX_QPS` | 单实例高德限流 | 默认 `3`，与当前账号 QPS 配额一致 |
