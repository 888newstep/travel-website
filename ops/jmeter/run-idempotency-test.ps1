...
param(
    ...
    [Parameter()]
    [ValidateSet('cold', 'warmup', 'steady', 'manual')]
...
$http200Elapsed = [long[]]@($mainRows | Where-Object { $_.responseCode -eq '200' } | ForEach-Object { [long]$_.elapsed })
$http409Elapsed = [long[]]@($mainRows | Where-Object { $_.responseCode -eq '409' } | ForEach-Object { [long]$_.elapsed })
$aggregateRow = $rows | Where-Object { $_.label -eq 'teardown:aggregate' } | Select-Object -Last 1
$aggregateMessage = if ($null -ne $aggregateRow) { $aggregateRow.responseMessage } else { $null }
$idempotencyOutcome = [ordered]@{
    original = 0
    replayed = 0
    conflict = 0
    unexpected = $mainRows.Count
}
if ($aggregateMessage -match '^original=(\d+),replayed=(\d+),conflict=(\d+),unexpected=(\d+)$') {
    $idempotencyOutcome.original = [int]$matches[1]
    $idempotencyOutcome.replayed = [int]$matches[2]
    $idempotencyOutcome.conflict = [int]$matches[3]
    $idempotencyOutcome.unexpected = [int]$matches[4]
}
...
$summary = [ordered]@{
    ...
    windowStartEpochMs = [long]$startTimestamp
    windowEndEpochMs = [long]$endTimestamp
    http200Latency = if ($http200Elapsed.Count -gt 0) {
        [ordered]@{
            count = $http200Elapsed.Count
            averageMs = [Math]::Round(($http200Elapsed | Measure-Object -Average).Average, 2)
            minMs = ($http200Elapsed | Measure-Object -Minimum).Minimum
            maxMs = ($http200Elapsed | Measure-Object -Maximum).Maximum
        }
    } else { $null }
    http409Latency = if ($http409Elapsed.Count -gt 0) {
        [ordered]@{
            count = $http409Elapsed.Count
            averageMs = [Math]::Round(($http409Elapsed | Measure-Object -Average).Average, 2)
            minMs = ($http409Elapsed | Measure-Object -Minimum).Minimum
            maxMs = ($http409Elapsed | Measure-Object -Maximum).Maximum
        }
    } else { $null }
