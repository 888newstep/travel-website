...
function Start-DisposableRedis {
    ...
    $arguments = @(
        ...
        '--save', '',
        '--dir', $OutputDirectory,
        '--dbfilename', 'comment-like-test.rdb',
        '--logfile', $LogPath
    )
    $startInfo = New-Object System.Diagnostics.ProcessStartInfo
...
function Start-CollectionService {
    param(
        [string]$JarPath,
        [string]$JwtSecret,
...
function Wait-ServiceReady {
    ...
    while ([DateTime]::UtcNow -lt $deadline) {
        if ($Process.HasExited) {
            ...
            throw "Collection service exited during startup. $tail"
        }
        try {
            $response = Invoke-WebRequest `
                    -Uri "$BaseUrl/route-comments/$CommentId/replies?page=1&size=1" `
    ...
    throw "Collection service did not become ready within $TimeoutSeconds seconds."
}
...
$testPlanPath = Join-Path $PSScriptRoot 'route-comment-like-concurrency.jmx'
...
$testUsernamePrefix = "jmeter_comment_like_$($runId.Substring(0, 12))"
$outputDirectory = Join-Path $repoRoot "run-logs\jmeter-comment-like\$timestamp"
...
$serviceLogPath = Join-Path $outputDirectory 'collection-service.log'
...
$userCsvPath = Join-Path $outputDirectory 'users.csv'
...
try {
    ...
    Invoke-MySqlScript (Join-Path $repoRoot 'backend\docs\infrastructure\USER_COLLECTION_ACTION_INDEX_MIGRATION.sql')
    Invoke-MySqlScript (Join-Path $repoRoot 'backend\docs\infrastructure\ROUTE_COMMENT_CONSISTENCY_MIGRATION.sql')

    $routeId = [int](Invoke-MySql 'SELECT MIN(id) FROM route;')
    if ($routeId -le 0) {
        throw 'No route exists for the temporary comment.'
    }

    $ownerUsername = "${testUsernamePrefix}_owner"
    $ownerIdText = Invoke-MySql @"
INSERT INTO user (username, password, user_type)
VALUES ('$ownerUsername', 'not-used-by-jmeter', 1);
SELECT LAST_INSERT_ID();
"@
    $ownerUserId = [int]($ownerIdText -split "`r?`n" | Select-Object -Last 1)

    $commentIdText = Invoke-MySql @"
INSERT INTO route_comment
    (route_id, user_id, rating, content, likes_count, is_anonymous, is_published, feedback_type)
VALUES
    ($routeId, $ownerUserId, 5.0, 'temporary jmeter comment like target', 0, FALSE, TRUE, 'comment');
SELECT LAST_INSERT_ID();
"@
    $commentId = [int]($commentIdText -split "`r?`n" | Select-Object -Last 1)

    $userRows = New-Object System.Collections.Generic.List[string]
    for ($index = 1; $index -le $Threads; $index++) {
        $username = "${testUsernamePrefix}_user_$($index.ToString('D4'))"
        $userRows.Add("('$username', 'not-used-by-jmeter', 1)")
    }
    Invoke-MySql ("INSERT INTO user (username, password, user_type) VALUES " + ($userRows -join ',') + ';') | Out-Null

    $createdUsers = @((Invoke-MySql "SELECT id, username FROM user WHERE username LIKE '${testUsernamePrefix}_user_%' ORDER BY username;") -split "`r?`n" | Where-Object { $_ })
    if ($createdUsers.Count -ne $Threads) {
        throw "Expected $Threads temporary liker users, found $($createdUsers.Count)."
    }

    $jwtSecret = [Guid]::NewGuid().ToString('N')
    $csvLines = New-Object System.Collections.Generic.List[string]
    foreach ($createdUser in $createdUsers) {
        $parts = $createdUser.Split("`t")
        if ($parts.Count -ne 2) {
            throw 'Unexpected temporary user query result.'
        }
        $userId = [int]$parts[0]
        $csvLines.Add("$userId,$(New-TestJwt $jwtSecret $userId)")
    }
    Write-Utf8NoBom $userCsvPath ($csvLines -join [Environment]::NewLine)
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Collection service build failed. See $buildLogPath"
    }
    $jar = Get-ChildItem (Join-Path $repoRoot 'backend\collection-service\target') -Filter '*.jar' -File |
            Where-Object { $_.Name -notmatch '\.original$' } |
            Sort-Object Length -Descending |
            Select-Object -First 1
    if ($null -eq $jar) {
        throw 'Collection service jar was not produced.'
    }

    $firstToken = ($csvLines[0] -split ',', 2)[1]
    $serviceProcess = Start-CollectionService $jar.FullName $jwtSecret $processMarker $serviceLogPath
    Wait-ServiceReady $serviceProcess $serviceBaseUrl $commentId $firstToken $StartupTimeoutSeconds $serviceLogPath

    $authorizationHeaders = @{ Authorization = "Bearer $firstToken" }
    $routeCommentsResponse = Invoke-WebRequest `
            -Uri "$serviceBaseUrl/route-comments/route/${routeId}?page=1&size=500" `
            -Headers $authorizationHeaders `
            -UseBasicParsing `
            -TimeoutSec 10
    $routeCommentsPayload = $routeCommentsResponse.Content | ConvertFrom-Json
    $routeComments = @($routeCommentsPayload.data)
    $wrongRouteComments = @($routeComments | Where-Object { [int]$_.routeId -ne $routeId })
    if ($routeCommentsResponse.StatusCode -ne 200 -or
            $routeCommentsPayload.success -ne $true -or
            $wrongRouteComments.Count -ne 0) {
        throw 'Route comment scope probe failed.'
    }

    $expectedStatistics = Invoke-MySql "SELECT COUNT(*), COALESCE(AVG(rating), 0) FROM route_comment WHERE route_id=$routeId AND is_published=TRUE AND reply_to IS NULL;"
    $expectedStatisticsParts = $expectedStatistics.Split("`t")
    if ($expectedStatisticsParts.Count -ne 2) {
        throw 'Unexpected route comment statistics baseline.'
    }
    $statisticsResponse = Invoke-WebRequest `
            -Uri "$serviceBaseUrl/route-comments/statistics/$routeId" `
            -Headers $authorizationHeaders `
            -UseBasicParsing `
            -TimeoutSec 10
    $statisticsPayload = $statisticsResponse.Content | ConvertFrom-Json
    $expectedTotalComments = [int]$expectedStatisticsParts[0]
    $expectedAverageRating = [double]$expectedStatisticsParts[1]
    $actualTotalComments = [int]$statisticsPayload.data.totalComments
    $actualAverageRating = [double]$statisticsPayload.data.averageRating
    $scopeProbe = [ordered]@{
        routeId = $routeId
        returnedComments = $routeComments.Count
        wrongRouteComments = $wrongRouteComments.Count
        statisticsHttpStatus = [int]$statisticsResponse.StatusCode
        statisticsSuccess = [bool]$statisticsPayload.success
        expectedTotalComments = $expectedTotalComments
        actualTotalComments = $actualTotalComments
        expectedAverageRating = [Math]::Round($expectedAverageRating, 8)
        actualAverageRating = [Math]::Round($actualAverageRating, 8)
    }
    if ($statisticsResponse.StatusCode -ne 200 -or
            $statisticsPayload.success -ne $true -or
            $actualTotalComments -ne $expectedTotalComments -or
            [Math]::Abs($actualAverageRating - $expectedAverageRating) -gt 0.0001) {
        throw "Route comment statistics probe failed: $($scopeProbe | ConvertTo-Json -Compress)"
    }
    ...
    $jmeterArguments = @(
        ...
        "-Jcomment_id=$commentId",
        "-Jthreads=$Threads",
        "-Juser_csv=$($userCsvPath.Replace('\', '/'))",
    ...
    $samples = @(Import-Csv -LiteralPath $resultPath | Where-Object { $_.label -eq 'comment-like:toggle' })
    if ($samples.Count -ne $Threads) {
        throw "Expected $Threads comment like samples, found $($samples.Count)."
    }
    ...
    if ($jmeterExitCode -ne 0 -or $failedSamples.Count -ne 0) {
        throw "JMeter comment like load failed: exitCode=$jmeterExitCode, failedSamples=$($failedSamples.Count)."
    }
...
SELECT
    ...
    (SELECT COUNT(*) FROM user_collection
     WHERE item_id=$commentId AND item_type='route_comment' AND collection_type='like'),
...
"@
    $parts = $row.Split("`t")
    if ($parts.Count -ne 3) {
        throw 'Unexpected comment like verification result.'
    }
    $databaseResult = [ordered]@{
        likeCount = [int]$parts[0]
        actionRows = [int]$parts[1]
        distinctUsers = [int]$parts[2]
    }
    if ($databaseResult.likeCount -ne $Threads -or
            $databaseResult.actionRows -ne $Threads -or
            $databaseResult.distinctUsers -ne $Threads) {
        throw "Database comment like verification failed: $($databaseResult | ConvertTo-Json -Compress)"
    }
    $success = $true
}
catch {
    ...
    if (Test-Path -LiteralPath $userCsvPath -PathType Leaf) {
        Remove-Item -LiteralPath $userCsvPath -Force -ErrorAction SilentlyContinue
    }
    try {
        if ($null -ne $commentId) {
            Invoke-MySql "DELETE FROM user_collection WHERE item_id=$commentId AND item_type='route_comment' AND collection_type='like'; DELETE FROM route_comment WHERE id=$commentId;" | Out-Null
        }
        Invoke-MySql "DELETE FROM user WHERE username LIKE '$testUsernamePrefix%';" | Out-Null
    }
    catch {
        if ($null -eq $failure) {
...
Write-Host "Route comment like concurrency test success: $success"
