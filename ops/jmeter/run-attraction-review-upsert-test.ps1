...
function Wait-ServiceReady {
    ...
    while ([DateTime]::UtcNow -lt $deadline) {
        if ($Process.HasExited) {
            ...
            throw "Attraction service exited during startup. $tail"
        }
        try {
            $response = Invoke-WebRequest `
                    -Uri "$BaseUrl/attractions/$TargetAttractionId" `
                    -UseBasicParsing `
                    -TimeoutSec 3
    ...
    throw "Attraction service did not become ready within $TimeoutSeconds seconds."
}

function Start-AttractionService {
    param(
        [string]$JarPath,
        [string]$JwtSecret,
...
$testPlanPath = Join-Path $PSScriptRoot 'attraction-review-upsert.jmx'
...
$outputDirectory = Join-Path $repoRoot "run-logs\jmeter-review\$timestamp"
...
$serviceLogPath = Join-Path $outputDirectory 'attraction-service.log'
...
$success = $false
$failure = $null
$previousToken = [Environment]::GetEnvironmentVariable('TRAVEL_JMETER_TOKEN', 'Process')
...
try {
    ...
    Invoke-MySqlScript (Join-Path $repoRoot 'backend\docs\infrastructure\ATTRACTION_REVIEW_IDEMPOTENCY_MIGRATION.sql')

    if ($AttractionId -eq 0) {
        $AttractionId = [int](Invoke-MySql 'SELECT MIN(id) FROM attraction;')
    }
    $attractionExists = Invoke-MySql "SELECT COUNT(*) FROM attraction WHERE id=$AttractionId;"
    if ($attractionExists -ne '1') {
        throw "Attraction does not exist: $AttractionId"
    }

    $testUsername = "jmeter_review_$($runId.Substring(0, 20))"
    $testUserIdText = Invoke-MySql @"
INSERT INTO user (username, password, user_type)
VALUES ('$testUsername', 'not-used-by-jmeter', 1);
SELECT LAST_INSERT_ID();
"@
    $testUserId = [int]($testUserIdText -split "`r?`n" | Select-Object -Last 1)

    $buildArguments = @(
        '-q',
        '-f', (Join-Path $repoRoot 'backend\pom.xml'),
        '-pl', 'attraction-service',
        '-am',
        '-DskipTests',
        'package'
    )
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Attraction service build failed. See $buildLogPath"
    }
    $jar = Get-ChildItem (Join-Path $repoRoot 'backend\attraction-service\target') -Filter '*.jar' -File |
            Where-Object { $_.Name -notmatch '\.original$' } |
            Sort-Object Length -Descending |
            Select-Object -First 1
    if ($null -eq $jar) {
        throw 'Attraction service jar was not produced.'
    }

    $jwtSecret = [Guid]::NewGuid().ToString('N')
    $token = New-TestJwt $jwtSecret $testUserId
    $serviceProcess = Start-AttractionService $jar.FullName $jwtSecret $processMarker $serviceLogPath
    Wait-ServiceReady $serviceProcess $serviceBaseUrl $AttractionId $StartupTimeoutSeconds $serviceLogPath

    [Environment]::SetEnvironmentVariable('TRAVEL_JMETER_TOKEN', $token, 'Process')
    $jmeterArguments = @(
        '-Xms512m',
        '-Xmx1g',
    ...
    $samples = @(Import-Csv -LiteralPath $resultPath | Where-Object { $_.label -eq 'review:upsert' })
    if ($samples.Count -ne $Threads) {
        throw "Expected $Threads review samples, found $($samples.Count)."
    }
    ...
    if ($jmeterExitCode -ne 0 -or $failedSamples.Count -ne 0) {
        throw "JMeter review load failed: exitCode=$jmeterExitCode, failedSamples=$($failedSamples.Count)."
    }

    $finalContent = "final-review-$runId"
    $finalBody = @{ rating = 5; content = $finalContent } | ConvertTo-Json -Compress
    $headers = @{
        Authorization = "Bearer $token"
        'Idempotency-Key' = "review-final-$runId"
    }
    $finalResponse = Invoke-WebRequest `
            -Uri "$serviceBaseUrl/attractions/$AttractionId/review" `
            -Method Post `
            -Headers $headers `
            -ContentType 'application/json' `
            -Body $finalBody `
            -UseBasicParsing `
            -TimeoutSec ([Math]::Ceiling($ResponseTimeoutMs / 1000.0))
    $finalResponseCode = [int]$finalResponse.StatusCode
    $finalPayload = $finalResponse.Content | ConvertFrom-Json
    if ($finalResponseCode -ne 200 -or $finalPayload.success -ne $true -or $finalPayload.code -ne 200) {
        throw 'Final deterministic review update failed.'
    }

    $row = Invoke-MySql "SELECT COUNT(*), MIN(id), MIN(rating), MIN(content) FROM attraction_review WHERE attraction_id=$AttractionId AND user_id=$testUserId;"
    $parts = $row.Split("`t")
    if ($parts.Count -ne 4) {
        throw 'Unexpected review verification result.'
    }
    $databaseResult = [ordered]@{
        rowCount = [int]$parts[0]
        reviewId = [long]$parts[1]
        rating = [int]$parts[2]
        content = $parts[3]
    }
    if ($databaseResult.rowCount -ne 1 -or
            $databaseResult.rating -ne 5 -or
            $databaseResult.content -ne $finalContent) {
        throw "Database upsert verification failed: $($databaseResult | ConvertTo-Json -Compress)"
    }
    $success = $true
}
catch {
    ...
    [Environment]::SetEnvironmentVariable('TRAVEL_JMETER_TOKEN', $previousToken, 'Process')
    ...
    if ($null -ne $testUserId) {
        try {
            Invoke-MySql "DELETE FROM attraction_review WHERE user_id=$testUserId; DELETE FROM user WHERE id=$testUserId;" | Out-Null
        }
        catch {
            if ($null -eq $failure) {
...
Write-Host "Attraction review upsert test success: $success"
