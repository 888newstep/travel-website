...
'@
    ...
    if ($results.Count -ne 8) {
        throw "Expected 8 live result markers, found $($results.Count)."
    }
    $success = $true
}
catch {
...
$summary = [ordered]@{
    ...
    note = 'Live test covers publish/consume, idempotency, retry/DLQ, corrected replay, consumer restart, forced connection recovery, and 100-message concurrent consumption.'
}
