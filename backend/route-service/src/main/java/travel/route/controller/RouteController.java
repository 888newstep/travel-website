...
public class RouteController {
    ...
    private final RouteVisitAnalyticsService routeVisitAnalyticsService;

    // ==================== CRUD操作 ====================

    @PostMapping
    @Operation(summary = "创建路线", description = "创建新的旅游路线")
    public Result<Route> createRoute(@Valid @RequestBody CreateRouteRequest request) {
        Integer userId = AuthenticatedUserSupport.requireIntegerUserId();
        Route route = toNewRoute(request);
    ...
    public Result<Route> getRoute(@PathVariable Integer id, HttpServletRequest request) {
        ...
        if (route != null) {
            try {
                routeVisitAnalyticsService.recordVisit(route, currentUserId, request);
                route.setViewCount((route.getViewCount() == null ? 0 : route.getViewCount()) + 1);
            } catch (RuntimeException exception) {
                log.warn("路线访问明细记录失败，不影响详情读取: routeId={}, error={}", id, exception.getMessage());
            }
    ...
    @Operation(summary = "读取可访问路线", description = "供已认证业务调用方校验路线可访问性，不记录访问统计")
    ...
    @Operation(summary = "获取路线访问分析", description = "路线所有者查看访问趋势、UV与回访率")
    public Result<RouteVisitAnalyticsResponse> getRouteVisitAnalytics(
            @PathVariable Integer id,
            @RequestParam(defaultValue = "30") int days) {
        Long userId = AuthenticatedUserSupport.requireUserId();
        routeService.checkRouteOwner(id.longValue(), userId);
        return Result.success("获取路线访问分析成功", routeVisitAnalyticsService.getAnalytics(id, days));
    }
    ...
    public Result<Route> updateRoute(@PathVariable Integer id,
        ...
        applyRouteChanges(existingRoute, request);
        existingRoute.setUpdatedAt(LocalDateTime.now());
        if (!routeService.updateById(existingRoute)) {
    ...
    public Result<Boolean> deleteRoute(@PathVariable Integer id) {
        ...
        if (!routeService.removeById(id)) {
            throw new BusinessException(ErrorCodeEnum.ROUTE_DELETE_FAILED);
        }
    ...
    public Result<List<Route>> getBatchRoutes(@RequestBody List<Integer> routeIds) {
        ...
        List<Route> routes = routeService.listByIds(routeIds).stream()
                .filter(route -> ("PUBLISHED".equals(route.getStatus()) && Boolean.TRUE.equals(route.getIsPublic()))
    ...
    public Result<Route> copyRoute(@PathVariable Integer id) {
        ...
        return Result.success("复制路线成功", routeLifecycleService.copyRoute(id, userId));
    }
    ...
    public Result<Boolean> setRouteVisibility(@PathVariable Integer id,
        ...
        if (Boolean.TRUE.equals(isPublic)) {
            routeLifecycleService.publish(id, userId.intValue());
        } else {
            routeLifecycleService.archive(id, userId.intValue());
        }
    ...
    private Route findReadableRoute(Integer routeId, Integer currentUserId) {
        Route route = routeService.getById(routeId);
        if (route == null) {
            return null;
        }
    ...
    private Route toNewRoute(CreateRouteRequest request) {
    ...
    private void applyRouteChanges(Route target, UpdateRouteRequest source) {
