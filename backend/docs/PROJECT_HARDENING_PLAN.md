...
- [x] 新增 `route_visit` 明细表，实现原子浏览计数、PV/UV、跨日回访率和按日趋势；访问者仅保存加盐 SHA-256 标识，不保存原始 IP/User-Agent。
- [x] 将收藏切换、评论点赞、分享访问/统计、通知和用户统计等稳定公共响应迁移为 DTO；供应商扩展和 AI 动态 JSON 保留 `Map`/`JsonNode`。
...
- [x] 路线创建和更新使用专用请求 DTO，服务端字段不可由客户端覆盖；CRUD 不存在或持久化失败统一返回非 200 业务错误。
...
- [x] 使用真实 `AMAP_API_KEY` 完成一条两景点路线和一条多景点路线的集成验收；Key 仅通过进程环境注入，证据已脱敏。
- [x] 配额失败、超时和无路径降级证据已由 `AMapRouteServiceTest` 记录；真实两点与多点外呼证据已由 `AMapRouteLiveIT` 记录。
- [x] 景点关键字搜索作为独立高德候选返回，不与本地评分、点评和业务 ID 混合。
- [x] 景点详情接入真实天气及餐饮、停车场、公厕、公交/地铁周边设施，并使用分类白名单和 100–5000 米半径校验。
- [x] 高德外呼增加默认 3 QPS 平滑限流、排队上限和现有缓存/舱壁协同保护；多实例全局限流边界已记录。
- [x] 路线详情新增类型化实时路况接口，复用相邻景点驾车聚合并执行路线可读权限校验。
...
- [x] 在云端 broker 完成真实业务消息的 confirm、消费落库、重复投递、retry/DLQ 和修正后死信回放。
- [x] 完成消费者停止/重启、强制连接重建恢复和 100 条唯一消息并发消费验收。
...
- [x] 建立 100 条唯一 RabbitMQ 消息并发消费场景：数据库落库 100 条，耗时 696ms，吞吐 143.68 msg/s。
- [x] 建立统一验收指标矩阵：HTTP 压测记录平均、P95、P99、吞吐、错误率和数据库事实；异步消息、故障演练及非缓存场景明确标记不适用指标，不伪造 Redis 命中率。
- [x] 新增不依赖 Nacos 的 8090–8095 全栈 E2E 启停脚本，按真实监听 PID 管理进程并输出启动证据。
- [x] 完成 62 项核心业务、权限、错误码和自动清理矩阵验收，真实高德搜索、天气、周边设施和路线交通均进入业务链路。
- [x] 收藏、评论和分享等跨服务读取改用认证的无副作用路线读取接口，不再因内部存在性校验污染路线访问统计。
- [x] 网关收藏幂等编排脚本显式启用幂等、关闭无关 Sentinel/XXL-Job，并使用临时备份表自动恢复测试前收藏基线。
- [x] 2026-08-23 重跑 100 并发：1 个 HTTP 200、99 个处理中 HTTP 409、0 非预期响应；最终唯一记录和基线恢复均通过。
- [x] 2026-08-23 路线访问分析复验：认证访问 3、匿名访问 2、PV 5、UV 3、无原始身份列，清理通过。
...
- `docs/performance/VERIFICATION_METRICS_MATRIX_20260822.md`
- `docs/infrastructure/AMAP_ROUTE_LIVE_VERIFICATION_20260823.md`
- `docs/showcase/ACCEPTANCE_REPORT_20260823.md`
...
本轮本地验证结果：前端 lint/build 通过，后端全量 compile 通过；2026-08-23 最终后端全量测试生成 71 份 Surefire 报告，共 295 项测试，0 failure、0 error、3 skipped。路线数据迁移执行后 `route_attractions=23`、跨城市关系为 0、`route_transport=0`，可稳定选出上海两点路线和北京三点路线；高德本地桩 8 项、路线联动测试及 2026-08-23 真实两点/多点驾车外呼均通过，最新脱敏证据位于 `run-logs/amap/20260823-175126/`。2026-08-20 使用 JMeter 5.6.3 完成路线收藏真实 100 并发验收：原始 HTTP 200 为 1，处理中 HTTP 409 为 99，失败样本和非预期 HTTP 响应均为 0；平均响应 3342.57ms，P95 4686ms，P99 4707ms，吞吐 20.08 次/秒；完成后重放响应和最终收藏状态通过，MySQL 业务记录数为 1，唯一索引验收通过。同日使用独立 Redis 16379、user-service 18091 和 collection-service 18094 完成故障演练：Redis 中断后写请求返回 HTTP 503，业务记录保持 1 条；Redis 重启后查询恢复为 HTTP 200；重复插入被 MySQL 1062 拒绝，记录数仍为 1。故障演练证据位于 `run-logs/redis-outage/20260820-113223-525/`，临时端口和进程均已清理。用户凭据日志脱敏回归测试同步通过，修复前生成的本地日志已完成脱敏。
