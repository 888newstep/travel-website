...
CREATE TABLE IF NOT EXISTS `route` (
    ...
    status        VARCHAR(20) NOT NULL DEFAULT 'DRAFT' COMMENT '路线状态: DRAFT/PUBLISHED/ARCHIVED',
    ...
    INDEX idx_status_public_city (status, is_public, city_id)
...
CREATE TABLE IF NOT EXISTS `route_share` (
    ...
    route_id           INT NULL COMMENT '路线ID，路线分享以外的类型可为空',
...
-- -------------------------------------------------------
UPDATE route SET status = 'PUBLISHED' WHERE is_public = TRUE;
