...
public class MessageProducerService {
    ...
    public MessageProducerService(
            ...
            @Value("${mq.outbox.enabled:false}") boolean outboxEnabled) {
        ...
        if (outboxEnabled && (!statusPersistenceEnabled || !reliableNotificationProducerEnabled
                || mqMessageStatusService == null || objectMapper == null)) {
            throw new IllegalArgumentException(
                    "mq.outbox.enabled requires reliable producer, status persistence, and message status dependencies");
        }
    }
    ...
    public MessageProducerService(RabbitTemplate rabbitTemplate) {
        this(rabbitTemplate, null, null, false, false, false);
    }
    ...
    public MessageProducerService(
        ...
        this(rabbitTemplate, mqMessageStatusService, objectMapper,
                statusPersistenceEnabled, false, false);
    }

    /** 保留旧的五参数构造函数，兼容现有单元测试和手动装配代码。 */
    public MessageProducerService(
            RabbitTemplate rabbitTemplate,
            MqMessageStatusService mqMessageStatusService,
            ObjectMapper objectMapper,
            ...
            boolean reliableNotificationProducerEnabled) {
        this(rabbitTemplate, mqMessageStatusService, objectMapper,
                statusPersistenceEnabled, reliableNotificationProducerEnabled, false);
    }
        ...
        try {
            ...
            if (outboxEnabled) {
                enqueueMessage(
                        ...
                        message);
                log.info("通知消息已写入 Outbox，等待异步投递: userId={}, type={}", userId, type);
                return;
            }
     ...
     */
    /**
     * 在业务事务内写入可靠通知 Outbox。该方法不会吞掉写入异常。
     */
    public void enqueueNotification(Integer userId, String type, String title, String content) {
        if (!reliableNotificationProducerEnabled) {
            return;
        }
        NotificationMessageVO message = new NotificationMessageVO(
                userId, type, title, content, Map.of(), System.currentTimeMillis());
        if (outboxEnabled) {
            enqueueMessage(
                    RabbitMQConfig.RELIABLE_NOTIFICATION_EXCHANGE,
                    RabbitMQConfig.RELIABLE_NOTIFICATION_ROUTING_KEY,
    ...
    private String enqueueMessage(String exchange, String routingKey, Object message) {
        if (mqMessageStatusService == null || objectMapper == null) {
            throw new IllegalStateException("Outbox dependencies are not available");
        }
        String messageId = UUID.randomUUID().toString();
        String messageType = message.getClass().getSimpleName();
        String payloadJson = serializePayload(message, messageId);
        mqMessageStatusService.enqueue(messageId, messageType, exchange, routingKey, payloadJson);
        return messageId;
    }
