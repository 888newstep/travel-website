...
public class HttpIdempotencyFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        long claimStartedAtNanos = performanceStageRecorder.start();
        try {
            claim = idempotencyService.tryClaim(scope, idempotencyKey, metadata);
            performanceStageRecorder.record(
                    "idempotency.claim",
                    claimStartedAtNanos,
                    claim.status().name().toLowerCase(Locale.ROOT));
        } catch (RuntimeException e) {
            performanceStageRecorder.record("idempotency.claim", claimStartedAtNanos, "error");
        ...
        try {
            ...
            if (request.isAsyncStarted()) {
                // 异步业务尚未完成，不能提前释放锁；等待 processing TTL 后由客户端重试。
                // 当前项目没有异步写接口，后续若引入，应通过 AsyncListener 在完成时持久化响应。
                log.warn("Async request cannot complete HTTP idempotency record in filter lifecycle: key={}",
                        claim.key());
            ...
            } else if (isRetryableFailure(capturedResponse)) {
                safeRelease(claim);
            } else {
                // 非 5xx 响应可能已经产生业务副作用，无法安全重试时保留锁直到 TTL 到期。
                log.warn("HTTP idempotency response is not replayable; keeping claim until TTL: key={}",
                        claim.key());
            }
    ...
    private boolean isCacheable(CapturedBodyResponse response) {
        ...
        if (status < 200 || (status >= 300 && status < 400) || status >= 500 || response.overflowed()) {
            return false;
        }
    ...
    private boolean isRetryableFailure(CapturedBodyResponse response) {
    ...
    private void safeComplete(
        ...
        try {
            boolean completed = idempotencyService.complete(claim, response);
            outcome = completed ? "completed" : "not-persisted";
            if (!completed) {
                // 业务已经执行成功，不能释放锁，否则重试可能再次产生业务副作用。
                log.warn("HTTP idempotency result was not persisted before claim expiry: key={}", claim.key());
            }
        } catch (RuntimeException e) {
            log.error("Failed to complete HTTP idempotency record: key={}", claim.key(), e);
        } finally {
            performanceStageRecorder.record("idempotency.complete", startedAtNanos, outcome);
        }
    }
    ...
    private void replay(
        ...
        if (body.length > maxResponseBodyBytes) {
            writeError(response, HttpServletResponse.SC_CONFLICT,
                    "The stored idempotency response is too large");
            return;
        }
        response.resetBuffer();
